...
function requestRoutes() {
    return axios.get(`${baseRoute}routes`)
    .then((response) => {
...
function requestDirections(route) {
    ...
    return axios.get(`${baseRoute}directions/${route}`)
    .then((response) => {
...
function requestStops(route, direction) {
    ...
    return axios.get(`${baseRoute}stops/${route}/${direction}`)
    .then((response) => {
        console.log('requestDirections response', response);
        if (response.status !== 200) {
            // error
            console.error(`Error: Could not find any bus stops for route ${route} and direction ${direction}`);
            // TODO: need to handle
        }
...
function requestStopInformation(route, direction, stop) {
    ...
    return axios.get(`${baseRoute}${route}/${direction}/${stop}`)
    .then((response) => {
        if (response.status !== 200) {
            // error
            console.error(`Error: Could not find any bus stops for route ${route} and direction ${direction}`);
...
export { requestRoutes, requestDirections, requestStops, requestStopInformation };
